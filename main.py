...
MOONSHOT_MAX_RSI      = float(os.getenv("MOONSHOT_MAX_RSI", "75"))  # reject overbought entries
...
MOONSHOT_MIN_VOL_RATIO = float(os.getenv("MOONSHOT_MIN_VOL_RATIO", "0.8"))  # current candle must be ≥ 80% of avg vol
...
def telegram(msg: str):
    ...
    for attempt in range(2):  # one retry — Telegram occasionally drops messages
        try:
            r = _get_session().post(url, json=payload, timeout=8)
            if not r.ok:
                log.warning(f"Telegram send failed (HTTP {r.status_code}): {r.text[:200]}")
                # Retry without HTML parsing in case of malformed tags
                if attempt == 0:
...
def find_moonshot_opportunity(tickers: pd.DataFrame, budget: float,
    ...
    def _score_moonshot(sym: str) -> dict | None:
        with _moonshot_scan_sem:
            ...
            vol_ratio = (float(volume.iloc[-1]) / avg_vol) if avg_vol > 0 else 1.0
            vol_score = min(30, (vol_ratio - 1) * 15) if vol_ratio > 1 else 0
            # Reject dead coins: if volume is a tiny fraction of average, no one is trading this.
            # RSI alone (e.g. RSI=8) can score 31pts on a coin with zero liquidity — this gate
            # prevents entering on data artifacts. 0.8× avg = at least some genuine activity.
