...
def _cmd_ask(question: str, balance: float):
    ...
    if answer:
        ...
        # Send answer as plain text — Haiku output can contain symbols that break
        # Telegram's HTML parser (e.g. RSI < 40, prices, code snippets).
        header  = f"🧠 Ask: {safe_q}\n━━━━━━━━━━━━━━━\n"
        # Strip any residual HTML tags from Haiku's answer
        clean_a = re.sub(r'<[^>]+>', '', answer)
        telegram((header + clean_a)[:4000])
    else:
